...
def BO_pred(acq_func, build_type='no spec', plot=False, return_='pred', append=False, init='rand'):
    ...
    if build_type == 'no spec':
        encoding = {'concentration':'numeric', 'temperature':'numeric'}
        matrices = {}
    ...
    # Include 2 components as an external matrix
...
# Test different init methods

def test_BO_rand():
    assert BO_pred('TS', return_='pred', init='rand')
    
def test_BO_pam():
    assert BO_pred('TS', return_='pred', init='pam')

def test_BO_kmeans():
    assert BO_pred('TS', return_='pred', init='kmeans')
